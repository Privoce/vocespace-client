...
export function roomUrlAfterLogin(spaceName: string, href: string) {
  ...
  for (const key of ['auth', 'data', 'details', 'token', 'fromServer'])
...
export function useRoomEntry({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        await latest.current.handlePreJoinSubmit({
          username: latest.current.userChoices.username,
          videoEnabled: false,
          audioEnabled: false,
          videoDeviceId: '',
          audioDeviceId: '',
        ...
        if (active.current)
          latest.current.messageApi.error(latest.current.t('msg.error.room.unexpect'));
        console.error(error);
      }
    }, 5000);
