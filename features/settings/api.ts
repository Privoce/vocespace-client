import { createApiUrl } from '@/lib/http/url';
import { connect_endpoint } from '@/lib/http/paths';
import { AIConf, CreateSpaceStrategy, HyperbeamConf, RTCConf, SMTPConf, VocespaceConfig } from '@/features/settings/config';

const CONF_API_URL = connect_endpoint('/api/conf');
export const getConf = async (hostToken?: string) => {
  const url = createApiUrl(CONF_API_URL);
  if (hostToken) {
