...
import { AuthType, TokenResult, SearchParams, PlatformUser, verifyPlatformUser, isSpaceManager, IdentityType } from '@/features/room/model';
 ...
 * 适合需要简单处理的场景
 */
export const getPlatformUserInfo = ({ user }: { user: ParticipantSettings }) => {
  const { auth } = user;
