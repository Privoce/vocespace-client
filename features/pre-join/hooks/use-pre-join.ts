...
import { PreJoinError,validatePreJoin } from '../validate-pre-join';
...
export function usePreJoin({
  ...
  const tracks = usePreviewTracks(
    {
      audio:
        audioEnabled && hasMicrophonePermission ? { deviceId: audioDeviceId } : false,
      video:
  ...
  const handleSubmit = async () => {
    ...
    setSpinning(true);
    try {
      const finalUserChoices = await validatePreJoin({ space, username, data,
        audioEnabled: audioEnabled && hasMicrophonePermission,
        videoEnabled: videoEnabled && hasCameraPermission, audioDeviceId, videoDeviceId });
      if (!active.current) return;
      setUsername(finalUserChoices.username);
      await onSubmit?.(finalUserChoices);
    } catch (error) {
      if (!active.current) return;
      messageApi.error(error instanceof PreJoinError ? t(error.messageKey) : t('msg.error.room.unexpect'));
  ...
  const play_sound = async () => {
    ...
    if (audio_play_ref.current?.paused) {
      ...
      try { await audio_play_ref.current.play(); setPlay(true); }
      catch { setPlay(false); }
    } else {
  ...
  const changeVolume = (value: number) => { setVolume(value); useUserStore.setState({ volume: value }); };
  const changeBlur = (value: number) => { setBlur(value); setVideoBlur(value); useUserStore.setState({ blur: value }); };
  const goHome = () => router.push('/new_space');
  return { t, device, permissions, contextHolder, spinning, loading, data, config, space,
    micLabel, camLabel, userLabel, joinLabel, videoTrack, videoEnabled, videoEl, facingMode,
    blurValue, audioEnabled, audioDeviceId, audioTrack, setAudioDeviceId, volume, changeVolume,
    audio_play_ref, play_sound, play, setPlay, videoDeviceId, setVideoDeviceId, blur, changeBlur,
    showLoginBtn, inputRef, username, setUsername, handleSubmit, goHome };
}
