...
import { TokenResult, IdentityType, RoomType } from '@/features/room/model';
import { ParticipantSettings } from '@/features/spaces/model';
 ...
 * 服务端处理平台用户信息的 Hook
 */
export const getPlatformUserInfoServer = ({user}: {user: ParticipantSettings}) => {
  const {auth} = user;
